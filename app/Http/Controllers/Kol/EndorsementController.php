...
class EndorsementController extends Controller
{
    ...
    public function index(Request $request): JsonResponse|View
    {
        ...
        if ($tab === 'active' || $tab === 'aktif') {
            $query->whereIn('status', ['assigned', 'in_progress', 'content_submitted', 'content_approved', 'content_rejected']);
        ...
        if (view()->exists('kol.endorsements.index')) {
            return view('kol.endorsements.index', [
    ...
    {
        $user = $request->user();
        if ($user && $endorsement->kol_profile_id !== $user->kolProfile?->id && ! $user->hasRole('superadmin')) {
        ...
        if (view()->exists('kol.endorsements.show')) {
            return view('kol.endorsements.show', [
