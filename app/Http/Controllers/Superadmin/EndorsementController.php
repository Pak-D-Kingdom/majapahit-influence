...
class EndorsementController extends Controller
{
    ...
    public function show(Endorsement $endorsement): View
    {
        ...
        return view('superadmin.endorsements.show', ['endorsement' => $endorsement->load(['campaign.brand', 'kolProfile.user', 'contentProofs.files', 'commission'])]);
    }
    ...
    public function edit(Endorsement $endorsement): View
    {
        ...
        return view('superadmin.endorsements.form', compact('endorsement'));
    }
    ...
    public function update(EndorsementRequest $request, Endorsement $endorsement): RedirectResponse
    {
        ...
        if ($oldStatus !== $data['status'] && ! in_array($data['status'], self::TRANSITIONS[$oldStatus] ?? [], true)) {
            abort(422, 'Perubahan status endorsement tidak valid.');
        }
        DB::transaction(function () use ($endorsement, $data, $oldStatus, $request): void {
            $endorsement->update($data);
            if ($oldStatus !== $data['status']) {
                AuditLog::log('endorsement_status_changed', 'endorsements', $endorsement->id, ['status' => $oldStatus], ['status' => $data['status']], $request->user());
            }
            if ($oldStatus !== $data['status']) {
                app(NotificationService::class)->send($endorsement->load('kolProfile.user')->kolProfile->user, 'endorsement_status_changed', 'Status endorsement diperbarui', 'Status endorsement berubah dari '.$oldStatus.' menjadi '.$data['status'].'.', route('kol.endorsements.show', $endorsement));
            }
        });
