    ...
    <form method="POST" enctype="multipart/form-data" action="{{ $mode === 'create' ? route('superadmin.campaigns.store') : route('superadmin.campaigns.update', $campaign) }}" class="space-y-6">
        ...
        <div class="rounded-2xl border border-[#421b13]/8 bg-white p-6 shadow-xs">
            ...
            <div class="mt-5 grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    <label class="block text-xs font-bold text-[#421b13] font-heading">
                        Tanggal Mulai <span class="text-[#d5282d]">*</span>
                    </label>
                    <input type="date" name="start_date" value="{{ old('start_date', $campaign->start_date?->format('Y-m-d') ?: date('Y-m-d')) }}" required class="mt-2 w-full rounded-xl border border-[#421b13]/15 bg-white px-3.5 py-2.5 text-sm text-[#421b13] focus:border-[#d57028] focus:ring-2 focus:ring-[#d57028]/20 focus:outline-hidden">
                </div>

                <div>
                    <label class="block text-xs font-bold text-[#421b13] font-heading">
                        Tanggal Selesai <span class="text-[#d5282d]">*</span>
                    </label>
                    <input type="date" name="end_date" value="{{ old('end_date', $campaign->end_date?->format('Y-m-d') ?: date('Y-m-d', strtotime('+30 days'))) }}" required class="mt-2 w-full rounded-xl border border-[#421b13]/15 bg-white px-3.5 py-2.5 text-sm text-[#421b13] focus:border-[#d57028] focus:ring-2 focus:ring-[#d57028]/20 focus:outline-hidden">
                </div>
                ...
                <div>
                    ...
                    <input type="number" min="0" step="50000" name="budget" value="{{ old('budget', (int)($campaign->budget ?? 10000000)) }}" required class="mt-2 w-full rounded-xl border border-[#421b13]/15 bg-white px-3.5 py-2.5 text-sm font-bold text-[#421b13] focus:border-[#d57028] focus:ring-2 focus:ring-[#d57028]/20 focus:outline-hidden">
                </div>

                <div>
                    <label class="block text-xs font-bold text-[#421b13] font-heading">
                        File Dokumen Brief <span class="text-xs font-normal text-[#765f58]">(PDF, DOCX, JPG, PNG - Maks 5 File)</span>
                    </label>
                    <input type="file" name="brief_files[]" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="mt-2 block w-full rounded-xl border border-dashed border-[#421b13]/20 p-2.5 text-xs text-[#765f58] file:mr-3 file:rounded-lg file:border-0 file:bg-[#d57028]/10 file:px-3 file:py-1.5 file:text-xs file:font-bold file:text-[#d57028]">
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-xs font-bold text-[#421b13] font-heading">
                        Deskripsi Singkat <span class="text-[#d5282d]">*</span>
                    </label>
                    <textarea name="description" rows="3" required placeholder="Jelaskan gambaran umum tujuan campaign ini..." class="mt-2 w-full rounded-xl border border-[#421b13]/15 bg-white px-3.5 py-2.5 text-sm text-[#421b13] focus:border-[#d57028] focus:ring-2 focus:ring-[#d57028]/20 focus:outline-hidden">{{ old('description', $campaign->description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="rounded-xl bg-gradient-to-r from-[#d57028] to-[#b86021] px-6 py-2.5 text-xs font-bold text-white shadow-xs transition hover:from-[#b86021] hover:to-[#934510] font-heading flex items-center gap-1.5">
