...
<main class="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
    <div class="w-full max-w-md">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <a href="{{ route('login') }}" class="text-sm font-semibold text-indigo-600">← Kembali ke login</a>
            <h1 class="mt-6 text-2xl font-bold text-slate-950">Lupa password?</h1>
            <p class="mt-2 text-sm leading-6 text-slate-500">Masukkan email akun Anda untuk menerima link reset password.</p>
            @if(session('success'))
            ...
            <form method="POST" action="{{ route('password.email') }}" class="mt-6 space-y-5">
                ...
                <button class="w-full rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800">Kirim link reset</button>
            </form>
        </div>
    </div>
</main>
