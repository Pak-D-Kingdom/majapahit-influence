        ...
        <nav class="space-y-1.5" aria-label="Navigasi Utama Brand">
            @php
                $menuItems = [
                    ...
                    ['route' => 'brand.notifications.index', 'label' => 'Notifikasi', 'icon' => 'bi-bell-fill'],
                ];
            @endphp
            ...
            @foreach ($menuItems as $item)
                @php
                    ...
                    $hasUnread = $item['route'] === 'brand.notifications.index' && auth()->check() && auth()->user()->unreadNotifications()->exists();
                @endphp
                <a href="{{ $url }}"
