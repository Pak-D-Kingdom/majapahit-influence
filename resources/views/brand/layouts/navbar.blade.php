<header class="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-[#421b13]/8 bg-[#fff9f4]/90 px-4 backdrop-blur-md transition sm:px-6 lg:px-8">
    <div class="flex items-center gap-3">
        <button id="brand-sidebar-toggle" type="button" class="rounded-xl p-2 text-[#765f58] hover:bg-[#f7eee8] hover:text-[#421b13] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d57028] lg:hidden" aria-label="Buka menu navigasi" aria-expanded="false" aria-controls="brand-dashboard-sidebar">
    ...
    <div class="flex items-center gap-3 sm:gap-4">
        <div class="flex items-center gap-3 pl-2">
            <div class="hidden sm:block text-right">
                <p class="text-xs font-bold text-[#421b13] font-heading leading-tight">{{ auth()->user()->brand->name ?? auth()->user()->name ?? 'Brand Partner' }}</p>
                <p class="text-[11px] text-[#765f58] leading-tight">Brand Partner</p>
            </div>
