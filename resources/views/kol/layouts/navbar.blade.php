<header class="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-md transition sm:px-6 lg:px-8 shadow-xs">
    <div class="flex items-center gap-3">
        <button id="kol-sidebar-toggle" type="button" class="rounded-xl p-2 text-slate-500 hover:bg-blue-50/60 hover:text-[#0c3685] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b64d4] lg:hidden" aria-label="Buka menu navigasi" aria-expanded="false" aria-controls="kol-dashboard-sidebar">
            <i class="bi bi-list text-2xl"></i>
        </button>
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-[#0b64d4] font-heading">
                <span>PORTAL CREATOR</span>
    ...
    <div class="flex items-center gap-3 sm:gap-4">
        ...
        <div class="flex items-center gap-3 pl-2 border-l border-slate-200">
            <div class="hidden sm:block text-right">
                <p class="text-xs font-bold text-[#0c3685] font-heading leading-tight">{{ auth()->user()->name ?? 'KOL Creator' }}</p>
