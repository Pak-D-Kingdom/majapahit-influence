    ...
    <div class="flex-1 overflow-y-auto px-4 py-6">
        <div class="mb-3 flex items-center justify-between px-3">
            ...
            <span class="inline-flex items-center gap-1 rounded-full bg-[#d57028]/20 px-2 py-0.5 text-[10px] font-medium text-[#fec200]">
                <span class="size-1.5 rounded-full bg-[#fec200] animate-pulse"></span>
                KOL Portal
            </span>
        </div>
        ...
        <nav class="space-y-1.5">
            ...
            @foreach ($menuItems as $item)
                ...
                @endphp
                <a href="{{ $url }}"
                   class="group flex items-center justify-between rounded-xl px-3.5 py-2.5 text-sm font-medium transition duration-150 {{ $isActive ? 'bg-gradient-to-r from-[#d57028] to-[#d5282d] text-white font-semibold shadow-md shadow-[#d57028]/25' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <div class="flex items-center gap-3">
                        <i class="bi {{ $item['icon'] }} text-base {{ $isActive ? 'text-white' : 'text-[#d57028] group-hover:text-white' }}"></i>
                        <span class="font-body">{{ $item['label'] }}</span>
                    </div>
                    @if ($isActive)
                        <i class="bi bi-chevron-right text-xs text-white/80"></i>
                    @endif
                </a>
            @endforeach
        </nav>
        ...
        <div class="rounded-xl border border-white/5 bg-[#31140d]/80 p-3.5 backdrop-blur-xs">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-0.5 truncate text-[11px] text-white/50">Tumbuh bersama brand</p>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}" class="mt-3 pt-2.5 border-t border-white/5">
                @csrf
                <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-lg bg-white/5 py-1.5 text-xs font-medium text-white/70 hover:bg-rose-500/20 hover:text-rose-300 transition">
